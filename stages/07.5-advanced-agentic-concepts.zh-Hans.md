...
| 7 | **Autonomy Gradients / Trust Layers** | Config（autonomy policy）| 不同任务给不同自主权（suggest / propose / execute）| [Antigravity CLI permission system](https://docs.claude.com/en/docs/agents-and-tools/claude-code/overview) |
...
|---|---|---|
| Context 200k 满 / Multi-agent context overflow | Progressive Disclosure + Memory Persistence | Skills `references/` / `GEMINI.md` `@-import` / `.ai/<task>` brief |
...
|---|---|---|---|---|
| 1 | **Legibility** | 把 agent 当新进工程师、为它优化 navigability（不是让人读懂 agent）| Repo + Types | Skill `references/` + AGENTS.md / GEMINI.md pattern |
| 2 | **System of Record** | 知识住 `docs/`、不住 prompt；100 行 entry map 指向深处 | Repo | `.coord/memory.yml` shared-memory + AGENTS.md / GEMINI.md |
...
- **对应 tool**：Antigravity CLI Skill 的 `references/` 机制 + AGENTS.md / GEMINI.md pattern
...
- **100 行 entry map**：AGENTS.md / GEMINI.md 只放"地图"（指向 `docs/` 各区的索引）、不放实际内容。
...
- **对应 tool**：`.coord/memory.yml`（[agent-shared-memory](https://github.com/WenyuChiou/agent-collab-skills) skill）+ AGENTS.md / GEMINI.md pattern
...
- **AGENTS.md / GEMINI.md 100 行**：只放最高层的"这个项目干什么 + 主要结构在哪"、不放细节。
...
- **Skill `references/` 机制**：Antigravity CLI 的 Skill 把详细 reference 放在 `references/` 子目录、agent 需要才 load。默认不进 prompt、需要才进。
- **`@-import` 语法**：GEMINI.md 可以写 `@docs/architecture.md` 指向深处、按需拉而不是预先塞。
...
- **跨 work boundary**：Repo + Types
- **对应 tool**：Antigravity CLI Skill 的 `references/` 机制（只在 agent 需要才 load）+ Codex `.ai/<task>.md` brief pattern（先读 brief 再决定深挖）
...
3. [Antigravity CLI — How Claude remembers your project](https://code.claude.com/docs/en/memory) ⭐⭐ — 涵盖原则 #2、GEMINI.md 4-tier hierarchy + `@-import` + AGENTS.md 互通
...
上面 5 原则对所有 agent 通用，但 **coding agent**（Antigravity CLI / Codex / Aider 这类）的 harness 有额外重量级需求，值得单独拆开看——因为 Stage 4 的 CodeAct、Stage 5 的 Antigravity CLI 生态、Stage 8 的 sandbox 全都从这条线长出来。

**三个 coding-agent 专属的 harness 元件**：

| 元件 | 为什么 coding agent 特别需要 | 对照 Stage |
|---|---|---|
| **文件系统 + repo 状态快照** | agent 改 code → 要能 diff / rollback / replay；不像 chat agent 改完就忘 | Stage 5 GEMINI.md hierarchy、Stage 7 Retry |
...
- **Aider / Antigravity CLI / Codex 三家对照**：同样是 coding agent，但 harness 取舍不同——Aider 走 git-commit-per-edit 的极简 repo 状态、Antigravity CLI 走 GEMINI.md + plan mode + subagent、Codex 走 cloud sandbox + harness 抽象。**读者要学的是"harness 取舍怎么影响 agent 行为"、不是记哪家 API**。

> 📚 **想深入 coding-agent harness 设计**：先读 [OpenAI — Harness Engineering](https://openai.com/index/harness-engineering/)（Codex case study）+ [Anthropic — Building Effective Agents](https://www.anthropic.com/engineering/building-effective-agents) 的 tool design 段；想看实作对照就跑 [Stage 5 Antigravity CLI 生态](05-gemini-skills-ecosystem.zh-Hans.md) + [Stage 8 Code Sandbox](08-agent-interfaces.zh-Hans.md)。
...
上面 coding-agent harness 提到 Opus 4.8 的 **Dynamic Workflows**（2026-05-28、Antigravity CLI research preview）。它值得单独拆开——因为它把 [Stage 4 教的 workflow vs agent 区分](04-agent-frameworks.zh-Hans.md#两个维度先分清楚workflow-vs-agent--single-vs-multi)整个塌缩掉，是理解"agent-authored orchestration"最好的活教材。
...
| 面向 | 事实（official Antigravity CLI docs 证实）|
|---|---|
...
| 平台 | Antigravity CLI feature（CLI / Desktop / IDE / headless / Agent SDK、v2.1.154+）。**不是** raw API（没有 `/v1/workflows`）|
...
> 📚 **权威来源**：[Antigravity CLI — Dynamic Workflows docs](https://code.claude.com/docs/en/workflows)（机制 + 16/1000 上限 + 触发方式的 canonical）· [Anthropic — Introducing Dynamic Workflows](https://claude.com/blog/introducing-dynamic-workflows-in-claude-code)（定位 + token 警告）· [Anthropic — Building Effective Agents](https://www.anthropic.com/research/building-effective-agents)（workflow vs agent + orchestrator-workers 根基）· [Opus 4.8 announcement](https://www.anthropic.com/news/claude-opus-4-8)。

### 📋 概念验证 prompt（self-quiz）

> 🛠️ **想直接动手写 SKILL.md / GEMINI.md？** 4 个实作用 prompt（audit 既有 / 生成新的）已经**移到 [Stage 5](05-gemini-skills-ecosystem.zh-Hans.md)**，那边才是读者真正开始动手写的地方：
> - [Stage 5.1  GEMINI.md 设计 prompts](05-gemini-skills-ecosystem.zh-Hans.md#-claudemd-设计-prompts依-5-原则)
> - [Stage 5.3  SKILL.md 设计 prompts](05-gemini-skills-ecosystem.zh-Hans.md#-skillmd-设计-prompts含-skill-creator-替代)
...
请出 5 道情境题、每题描述一个真实的 SKILL.md / GEMINI.md 设计决定（例如“我把所有 examples 都塞 SKILL.md 主档、< 1000 行”），问**违反哪一条原则 + 该怎么修**。

一次出 1 题、等我回答后给反馈、再出下一题。最后给总成绩。
```

→ **建议用法**：学完上面 5 条原则后跑这个 quiz，确认你真的抓到 concept；真正写 SKILL.md / GEMINI.md 的 prompt，请回到 [Stage 5](05-gemini-skills-ecosystem.zh-Hans.md)。
...
4. **Encode 到 Skills + GEMINI.md**：用 Stage 5 的 4 个 prompt 动手写——GEMINI.md audit / generate（[Stage 5.1](05-gemini-skills-ecosystem.zh-Hans.md#51--claude-code-基础)）+ SKILL.md audit / generate（[Stage 5.3](05-gemini-skills-ecosystem.zh-Hans.md#53--skillsclaude-code-的行为层-claude-code-生态最关键的一层)）。
...
> 📋 **学完进阶概念回头看 synthesis** → [Stage 5 §🗺️ 7-Layer Architecture Map](05-gemini-skills-ecosystem.zh-Hans.md#-7-layer-architecture-map先看这张图再读-51-56)（Antigravity CLI 7 个 primitive + 3 个工程学 discipline 整合图）
