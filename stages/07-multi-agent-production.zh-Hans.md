...
|---|---|---|
| **Stage 5.5 Subagents** | Antigravity CLI 原生 subagent 机制（markdown-based、不写程序）| 通用 multi-agent framework（autogen / crewAI / langgraph、跨 vendor）|
| **Stage 5.6 Antigravity CLI source** | Antigravity CLI source 解剖（reference implementation case study）| Harness engineering 通则（不绑特定 vendor）|
...
7. [**ZhangHanDong/harness-engineering-from-cc-to-ai-coding**](https://github.com/ZhangHanDong/harness-engineering-from-cc-to-ai-coding)（★ 1.5k+）— 从 Antigravity CLI 源码学 harness 设计（中文）
...
| **Safety layer** | permission prompts、sandboxed exec、destructive op 拦截 | （Antigravity CLI 内建、SDK 可自定义）|
...
- **Antigravity CLI 整个 runtime** — 是 reference harness 实现。**读 source 练习见 [Stage 5.6](05-gemini-skills-ecosystem.md#56--claude-code-source-解剖reference-harness-implementation-track-b-必看)**（clone `claude-agent-sdk-python` 解剖 main loop + 上表前 6 个 runtime 元件位置；第 7 个 Eval harness 是外挂、第 8 个 Cost / Latency 是 cross-cutting、见下方深入段）
...
- 在 Antigravity CLI / Cursor 设置 prompt caching，daily session 省 50-90% cost
...
| **自架开源 LLM**（取代付费 API）| [vLLM](https://github.com/vllm-project/vllm) | 高吞吐量、★ 89k+ |
...
| | [claude-agent-sdk-python](https://github.com/anthropics/claude-agent-sdk-python) ⭐ | ⭐⭐⭐⭐⭐ | 打造 Claude-based agent 而非只 API | 内建 tool use loop / file access / sandbox / subagent 编排；跟 Antigravity CLI 同 runtime、想看内部运作直接读 source。★ 7.9k+、MIT |
...
| | [vLLM](https://github.com/vllm-project/vllm) | ⭐⭐⭐⭐ | 自架开源 LLM 取代付费 API | 高吞吐量 LLM serving、Llama / Qwen 等。★ 89k+、Apache 2.0 |
...
> 🌳 **Claude 原生 subagent 机制**（不用 framework 也能 multi-agent）见 [Stage 5.5](05-gemini-skills-ecosystem.zh-Hans.md#55--subagentsclaude-code-原生-multi-agent-机制-2025-新功能)。本 stage 重 framework / production；Stage 5.5 重 markdown-based subagent 编排。
