...
| 7 | **Autonomy Gradients / Trust Layers** | Config（autonomy policy）| agent 不同任務有不同自主權（suggest / propose / execute）| [Antigravity CLI permission system](https://docs.claude.com/en/docs/agents-and-tools/claude-code/overview) |
...
|---|---|---|
| Context 200k 滿 / Multi-agent context overflow | Progressive Disclosure + Memory Persistence | Skills `references/` / `GEMINI.md` `@-import` / `.ai/<task>` brief |
...
|---|---|---|---|---|
| 1 | **Legibility** | 把 agent 當新進工程師、為它優化 navigability（不是讓人讀懂 agent） | Repo + Types | Skill `references/` + AGENTS.md / GEMINI.md pattern |
| 2 | **System of Record** | 知識住 `docs/`、不住 prompt；100 行 entry map 指向深處 | Repo | `.coord/memory.yml` shared-memory + AGENTS.md / GEMINI.md |
...
- **對應 tool**：Antigravity CLI Skill 的 `references/` 機制 + AGENTS.md / GEMINI.md pattern
...
- **100 行 entry map**：AGENTS.md / GEMINI.md 只放「地圖」（指向 `docs/` 各區的索引）、不放實際內容。
...
- **對應 tool**：`.coord/memory.yml`（[agent-shared-memory](https://github.com/WenyuChiou/agent-collab-skills) skill）+ AGENTS.md / GEMINI.md pattern
...
- **AGENTS.md / GEMINI.md 100 行**：只放最高層的「這個專案幹什麼 + 主要結構在哪」、不放細節。
...
- **Skill `references/` 機制**：Antigravity CLI 的 Skill 把詳細 reference 放在 `references/` 子目錄、agent 需要才 load。預設不進 prompt、需要才進。
- **`@-import` 語法**：GEMINI.md 可以寫 `@docs/architecture.md` 指向深處、按需拉而不是預先塞。
...
- **跨 work boundary**：Repo + Types
- **對應 tool**：Antigravity CLI Skill 的 `references/` 機制（只在 agent 需要才 load）+ Codex `.ai/<task>.md` brief pattern（先讀 brief 再決定深挖）
...
3. [Antigravity CLI — How Claude remembers your project](https://code.claude.com/docs/en/memory) ⭐⭐ — 涵蓋原則 #2、GEMINI.md 4-tier hierarchy + `@-import` + AGENTS.md 互通
...
上面 5 原則對所有 agent 通用，但 **coding agent**（Antigravity CLI / Codex / Aider 這類）的 harness 有額外重量級需求，值得單獨拆開看——因為 Stage 4 的 CodeAct、Stage 5 的 Antigravity CLI 生態、Stage 8 的 sandbox 全都從這條線長出來。

**三個 coding-agent 專屬的 harness 元件**：

| 元件 | 為什麼 coding agent 特別需要 | 對照 Stage |
|---|---|---|
| **檔案系統 + repo 狀態快照** | agent 改 code → 要能 diff / rollback / replay；不像 chat agent 改完就忘 | Stage 5 GEMINI.md hierarchy、Stage 7 Retry |
...
- **OpenAI Agents SDK 2026-04 更新**把 sandbox（7 個 provider）+ harness 抽象層內建——[Stage 4 已標註](04-agent-frameworks.md)這是「production coding agent 首次 architecturally sound」。harness 不再是各家自己拼、開始有共用抽象。
- **Aider / Antigravity CLI / Codex 三家對照**：同樣是 coding agent，但 harness 取捨不同——Aider 走 git-commit-per-edit 的極簡 repo 狀態、Antigravity CLI 走 GEMINI.md + plan mode + subagent、Codex 走 cloud sandbox + harness 抽象。**讀者要學的是「harness 取捨怎麼影響 agent 行為」、不是記哪家 API**。

> 📚 **想深入 coding-agent harness 設計**：先讀 [OpenAI — Harness Engineering](https://openai.com/index/harness-engineering/)（Codex case study）+ [Anthropic — Building Effective Agents](https://www.anthropic.com/engineering/building-effective-agents) 的 tool design 段；想看實作對照就跑 [Stage 5 Antigravity CLI 生態](05-gemini-skills-ecosystem.md) + [Stage 8 Code Sandbox](08-agent-interfaces.md)。
...
上面 coding-agent harness 提到 Opus 4.8 的 **Dynamic Workflows**（2026-05-28、Antigravity CLI research preview）。它值得單獨拆開——因為它把 [Stage 4 教的 workflow vs agent 區分](04-agent-frameworks.md#兩個維度先分清楚workflow-vs-agent--single-vs-multi)整個塌縮掉，是理解「agent-authored orchestration」最好的活教材。
...
| 面向 | 事實（official Antigravity CLI docs 證實）|
|---|---|
...
| 平台 | Antigravity CLI feature（CLI / Desktop / IDE / headless / Agent SDK、v2.1.154+）。**不是** raw API（沒有 `/v1/workflows`）|
...
請出 5 道情境題、每題描述一個真實的 SKILL.md / GEMINI.md 設計決定（例如「我把所有 examples 都塞 SKILL.md 主檔、< 1000 行」），問**違反哪一條原則 + 該怎麼修**。

一次出 1 題、等我回答後給回饋、再出下一題。最後給總成績。
```

→ **建議用法**：學完上面 5 個原則後跑這個 quiz 確認你抓到 concept；實作面（寫 SKILL.md / GEMINI.md）的 prompt 在 [Stage 5](05-gemini-skills-ecosystem.md)。
...
4. **Encode 到 Skills + GEMINI.md**：用 Stage 5 的 4 個 prompt 動手寫——GEMINI.md audit / generate（[Stage 5.1](05-gemini-skills-ecosystem.md#51--claude-code-基礎)）+ SKILL.md audit / generate（[Stage 5.3](05-gemini-skills-ecosystem.md#53--skillsclaude-code-的行為層-claude-code-生態最關鍵的一層)）。
...
> 📋 **學完進階概念回頭看 synthesis** → [Stage 5 §🗺️ 7-Layer Architecture Map](05-gemini-skills-ecosystem.md#-7-layer-architecture-map先看這張圖再讀-51-56)（Antigravity CLI 7 個 primitive + 3 個工程學 discipline 整合圖）
