...
- 多數 AI agent 使用者**沒在自己寫 agent**——他們是 Antigravity CLI / Cursor / ChatGPT 重度使用者
...
|---|---|---|
| A1 | CLI 入門 + 選擇 | → 最終 A1 |
| A2 | Workflow（GEMINI.md / slash command / 任務拆解 / portable prompt） | → 最終 A2 |
...
- **A2**：Workflow Patterns（GEMINI.md / slash command / 多步拆解 / portable prompt）
...
Stage 5（Antigravity CLI 生態）兩條軌都會碰到：
- Track A：A2 用 5.1（Antigravity CLI 基礎）；A3 用 5.2（MCP）+ 選擇性用到 5.3（Skills）跟 5.4（Plugins）——A3 的 動手練習 CLI-12 會教 plugin 打包。讀的角度是「**怎麼用 Antigravity CLI 把工作做好**」
- Track B：把整個 Stage 5 當「**Antigravity CLI 內部運作**」的深度學，從 5.1 完整走到 5.4
...
### 太少（5 stage）的問題
要把 9 個概念塞 5 個 stage：API 基礎 / prompt / tool use / framework / Antigravity CLI 生態 / memory / RAG / multi-agent / agent 操作介面（Computer · Browser Use）。塞下去結果是有的 stage 太擠（譬如 framework + Antigravity CLI 擠一起，3-4 週的內容硬塞 1 stage），讀者跳不過去。
...
| **5** ⭐⭐ Antigravity CLI 生態 | 「Antigravity CLI 生態系怎麼吃？」 | MCP / Skills / Plugins / Marketplace 4 個 sub-stage |
...
### 為什麼 4 → 5 → 6 → 7 → 8 是這順序
- 4 framework 後 → 5 Antigravity CLI 生態（為什麼 Antigravity CLI 是核心？因為它把 5.1-5.4 的概念集成在一個工具裡）
...
### 1. 這 stage 是 Antigravity CLI 使用者的核心
Repo 名字是 `awesome-agentic-ai-zh`，受眾偏 Antigravity CLI 使用者。Stage 5 是這個生態的完整教學——不會這 stage 就不算懂 Antigravity CLI。
