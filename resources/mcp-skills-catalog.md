...
> 把 Antigravity CLI（或其他 CLI agent）接到你已經在用的工具，不用反覆切換視窗。本頁是 62 個分類整理過的 MCP server / Claude Skill / 整合範例（含研究工作流 + multi-LLM delegation 兩個專屬區）。
...
- **想看 MCP / Skills / Plugins 是什麼**：先看 [README 三個核心用語](../README.md#-快速開始)，再看 [Stage 5 — Antigravity CLI 生態系](../stages/05-gemini-skills-ecosystem.md)
- **想看 動手練習 怎麼裝、怎麼測**：看 [Stage 5.2 (MCP)](../stages/05-gemini-skills-ecosystem.md#52--mcpmodel-context-protocol-基礎) 跟 [Stage 5.3 (Skills)](../stages/05-gemini-skills-ecosystem.md#53--skillsclaude-code-的行為層-claude-code-生態最關鍵的一層)
...
**適合誰**：Obsidian 重度使用者，想用 Antigravity CLI 整理 daily note、自動 link、跨檔搜尋。
...
**教什麼**：Antigravity CLI Skill，用瀏覽器自動化操作 NotebookLM、查詢上傳文件，回覆帶 citation。
**適合誰**：用 NotebookLM 管 paper 跟研究筆記，但想在 Antigravity CLI 一條 prompt 直接查的人。
**備註**：需要 Google 帳號登入授權。
...
**適合誰**：所有 Antigravity CLI 使用者——直接 `claude skill install` 就能讓 Claude 讀寫 Office 檔。
...
**適合誰**：所有 GitHub 使用者；Antigravity CLI 接上去後 PR review、issue triage、release notes 都能跑。
...
**教什麼**：把 codebase / SQL schemas / R scripts / shell scripts / docs / papers / images / videos 變成 queryable knowledge graph 的 AI coding skill。Antigravity CLI、Codex、OpenCode、Cursor、Gemini CLI 都能接。
...
**備註**：Playwright 官方出，最 robust。**Antigravity CLI 接 web 自動化的不錯選項**。
...
**教什麼**：把 Figma layout 資訊送給 coding agent——讀設計稿、提元件結構，給 Cursor / Antigravity CLI 寫對應的 React component。
...
**教什麼**：MCP server + Antigravity CLI Skill，real-time canvas sync，可建立 / 編輯 / 匯出。
...
**備註**：「上週這個 error 的 stack trace 給我看」直接問 Antigravity CLI。
...
**適合誰**：想用 Antigravity CLI 控播放清單、做語音 / 文字 → 音樂的整合者。
...
**適合誰**：用飛書 / Lark 寫文件的中文使用者，要把 Lark 內容跟 Antigravity CLI 串起來。
...
**適合誰**：中文圈使用者要找一個替代 Antigravity CLI / OpenAI Operator 等級的 all-in-one agent；對中國大陸服務（網易、釘釘等）整合需求高的場景。
**備註**：產品式 agent（不是 Skill / MCP）；跟 Antigravity CLI / Codex 互為替代，不是搭配。
...
**備註**：MCP 相容是亮點——可以直接接到 Antigravity CLI 等 host；維護節奏正常（last commit 2026-03）。
...
**適合誰**：以 DeepSeek 為主力 LLM 的中文開發者；想用終端工具但不需要 Antigravity CLI 全家桶的人。
...
**適合誰**：用 Antigravity CLI / Codex / OpenClaw 做投研或量化分析的中文開發者；不想自己刻資料抓取邏輯的人。
**備註**：一條 `curl` + `pip install` 即可啟用；中國 A 股資料類 Skill 中星星數最高的社群實作。相容 Antigravity CLI、Codex、OpenClaw。
...
**適合誰**：用 Antigravity CLI / Claude Desktop 做個人理財分析、想讓 AI 找出被動收入機會的人。hosted remote MCP server 範例——直接 plug URL、0 安裝、適合 Stage 5 學完 MCP 概念後拿來實驗 hosted vs self-hosted 差異。
...
**教什麼**：14 個 Antigravity CLI skills 涵蓋常見研究任務——文獻分流、研究設計、project context、論文撰寫、multi-AI delegation。打包成 5-plugin marketplace，一個指令安裝。
...
**教什麼**：嚴謹學術論文寫作 / 修改 / 投稿的 Antigravity CLI skill。Field-agnostic，可用 per-paper journal_format.md 跟 style_overrides.md 客製規則。
...
**適合誰**：用 Zotero 管文獻、想讓 Antigravity CLI 直接整理 library 的研究者。
**備註**：跟 [`MuiseDestiny/zotero-gpt`](https://github.com/MuiseDestiny/zotero-gpt) 的差別——後者是 Zotero plugin（在 Zotero 裡 chat），這份是 CLI / Skill（從 Antigravity CLI 操作 Zotero）。
...
**教什麼**：Antigravity CLI skill 把 Codex CLI 當 execution specialist——大量檔案 refactor、batch edits、boilerplate 生成、wrapper-based 實作密集任務。Claude 寫 plan + review，Codex 執行。
...
**教什麼**：Antigravity CLI skill 把 Gemini CLI 當 long-form / large-context / CJK 執行者——百萬 token context window、中文長文 draft、second-opinion review。Claude 出大綱跟 critique，Gemini 寫長文。
...
**教什麼**：Antigravity CLI marketplace for multi-agent collaboration——task splitter、output reconciler、adversarial debate、shared memory、acceptance gate。跟 codex-delegate / gemini-delegate 組合用。
