...
→ Antigravity CLI 第一次装的详细步骤 → 本指南 D
...
## D — 第一次装 Antigravity CLI（约 10 分钟；Stage 5 / for-developer 会用到）

### 先装 Node.js

> 💡 **Node.js 是什么**：跑 JavaScript 的 runtime（类似 Python interpreter 但是给 JS 用）。**`npm`** 是它附带的“包管理器”（package manager）——跟 Python 的 `pip` 同角色、用来安装别人写好的工具（如下面的 Antigravity CLI）。`npm install -g X` 表示“全局安装 X，之后在任何文件夹都能用”。
...
在你的 project 根目录建立 `GEMINI.md`。Antigravity CLI 启动时会读它，理解你希望它怎么协助。
...
Skill 是 Antigravity CLI 的“可复用 prompt 包”。当你的消息符合描述，Antigravity CLI 会自动加载那份指示。
...
> 想看更完整的 Skill 设计：看 [Stage 5.3 — Skills](../stages/05-gemini-skills-ecosystem.zh-Hans.md#53--skillsclaude-code-的行为层-claude-code-生态最关键的一层)。
...
| 想看 Antigravity CLI 完整生态 | [Stage 5 — Antigravity CLI 生态](../stages/05-gemini-skills-ecosystem.zh-Hans.md) |
