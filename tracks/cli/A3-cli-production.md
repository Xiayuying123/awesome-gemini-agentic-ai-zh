...
- 設定 GitHub Actions 自動跑 Antigravity CLI（PR review、release notes 等）
...
- 完成 [A2](A2-cli-workflow.md)：寫過 production GEMINI.md、會寫 slash command、跑過多步驟拆解
- 對 GitHub Actions / CI 基礎熟悉（會看 `.yml` workflow）
- 對 MCP 概念有印象（沒有的話先翻 [Stage 5.2](../../stages/05-gemini-skills-ecosystem.md#52--mcpmodel-context-protocol-基礎)）

沒到的話 → 補完 [A1](A1-cli-intro.md) + [A2](A2-cli-workflow.md)。A3 是「組合所有前面學的 → 接到 production」、跳級會看不懂。

## 📚 必修閱讀

1. [**Stage 5.2 — MCP（Model Context Protocol）**](../../stages/05-gemini-skills-ecosystem.md#52--mcpmodel-context-protocol-基礎) — MCP 概念跟基礎
...
照 [Stage 5.2 練習：MCP client](../../stages/05-gemini-skills-ecosystem.md#52--mcpmodel-context-protocol-基礎) 的步驟，把至少一個有用的 MCP server 接到你的 CLI：
...
- 跑：在 GH Actions runner 內執行 Antigravity CLI（或 Codex），給它 `git diff` + 你的 `.claude/commands/review.md`
- 輸出：PR comment
...
把你的 `.claude/commands/` 跟 `GEMINI.md` 打包成 plugin，發布到內部 marketplace 或 GitHub。Team 其他人 `claude plugin install` 之後就有同樣的工作流。
- Skill / plugin 細節見 [Stage 5.3 + 5.4](../../stages/05-gemini-skills-ecosystem.md)
...
> 💡 **第一次聽到 subagent？** 一句話：**subagent = 主 Claude session spawn 出來的「子 Claude」**、有自己獨立的 context、跑完回報結果。**派遣（dispatch）**就是叫 subagent 去做事——像派任務給同事。完整概念 → [Stage 5.5](../../stages/05-gemini-skills-ecosystem.md#55--subagentsclaude-code-原生-multi-agent-機制-2025-新功能)。

- **When**：寫了大改動要 commit 前 / 進新 repo 不熟結構 / 想跑 LLM-as-judge 自動評估 / 4 個目標要做同樣審查
- **Do**：呼叫 Antigravity CLI **內建** subagent（不用自己寫任何檔案）：
...
- **Read more**：
  - [Stage 5.5 Subagents](../../stages/05-gemini-skills-ecosystem.md#55--subagentsclaude-code-原生-multi-agent-機制-2025-新功能)（完整理論 + decision table）
...
- **Read more**：
  ...
  | 內部 | [Stage 5.5 Subagents](../../stages/05-gemini-skills-ecosystem.md#55--subagentsclaude-code-原生-multi-agent-機制-2025-新功能) + 動手練習 CLI-10 |
...
- **When**：GEMINI.md / SKILL.md rule 寫了沒人 enforce、preset YAML 加了不知道有沒有效
- **Do**：故意 break 一條 rule 跑 acceptance gate 看抓不抓得到（chaos test）；`docs/` 當 single source、GEMINI.md 只當 entry map
...
|---|---|---|---|---|
| **MCP server collection**<br>（接 CLI 用） | [modelcontextprotocol/servers](https://github.com/modelcontextprotocol/servers) | ⭐⭐⭐⭐⭐ | 第一個 MCP 從 reference 學起 | 官方 reference servers（filesystem / github / sqlite / git / time / fetch / memory / sequential-thinking），★ 85k+。詳見 [Stage 5.2](../../stages/05-gemini-skills-ecosystem.md#52--mcpmodel-context-protocol-基礎) |
...
| **Observability + Cost** | [langfuse/langfuse](https://github.com/langfuse/langfuse) | ⭐⭐⭐⭐⭐ | 想把 trace / cost / session 都接起來 | open source LLM observability，★ 33k+。詳見 [Stage 7 常用推薦](../../stages/07-multi-agent-production.md#-常用-multi-agent--production-工具推薦按用途分類) |
...
| **Production CLI workflow 範本** | [obra/superpowers](https://github.com/obra/superpowers) | ⭐⭐⭐⭐ | 看完整實際在用的 workflow 長什麼樣 | 整套 production-ready skill collection、★ 272k+。看別人怎麼把 CLI workflow 做完整 |
...
- [ ] 把你的 GEMINI.md / commands 打包過至少一次（即使只有自己用）
...
1. **加深 CLI workflow**（持續優化你的 setup）
   ...
   - 跟你 team 分享 GEMINI.md / skills

2. **跨到 Track B**（學怎麼寫自己的 agent）
   - Stage 3-4 學 tool use + framework
   - Stage 5 深挖 Antigravity CLI 內部運作
