...
- 设置 GitHub Actions 自动跑 Antigravity CLI（PR review、release notes 等）
- 加 observability（trace、cost、latency）到 CLI workflow
- 规划 cost budget，知道大 task 会花多少 token

## 📚 必修阅读

1. [**Stage 5.2 — MCP（Model Context Protocol）**](../../stages/05-gemini-skills-ecosystem.zh-Hans.md#52--mcpmodel-context-protocol-基础) — MCP 概念跟基础
...
照 [Stage 5.2 练习：MCP client](../../stages/05-gemini-skills-ecosystem.zh-Hans.md#动手练习) 的步骤，把至少一个有用的 MCP server 接到你的 CLI：
...
- 跑：在 GH Actions runner 内执行 Antigravity CLI（或 Codex），给它 `git diff` + 你的 `.claude/commands/review.zh-Hans.md`
- 输出：PR comment
...
- Skill / plugin 细节见 [Stage 5.3 + 5.4](../../stages/05-gemini-skills-ecosystem.zh-Hans.md)
...
> 💡 **第一次听到 subagent？** 一句话：**subagent = 主 Claude session spawn 出来的“子 Claude”**，有自己独立的 context，跑完回报结果。**派遣（dispatch）**就是叫 subagent 去做事——像派任务给同事。完整概念 → [Stage 5.5](../../stages/05-gemini-skills-ecosystem.zh-Hans.md#55--subagentsclaude-code-原生-multi-agent-机制-2025-新功能)。

- **When**：写了大改动要 commit 前 / 进新 repo 不熟结构 / 想跑 LLM-as-judge 自动评估 / 4 个目标要做同样审查
- **Do**：调用 Antigravity CLI **内置** subagent（不用自己写任何文件）：
...
- **Read more**：
  - [Stage 5.5 Subagents](../../stages/05-gemini-skills-ecosystem.zh-Hans.md#55--subagentsclaude-code-原生-multi-agent-机制-2025-新功能)（完整理论 + decision table）
...
- **Read more**：
  ...
  | 内部 | [Stage 5.5 Subagents](../../stages/05-gemini-skills-ecosystem.zh-Hans.md#55--subagentsclaude-code-原生-multi-agent-机制-2025-新功能) + 动手练习 CLI-10 |
...
- **When**：GEMINI.md / SKILL.md rule 写了但没人 enforce，preset YAML 加了也不知道有没有效
- **Do**：故意 break 一条 rule 跑 acceptance gate 看抓不抓得到（chaos test）；`docs/` 当 single source，GEMINI.md 只当 entry map
...
> 详见 [Stage 5.2](../../stages/05-gemini-skills-ecosystem.zh-Hans.md#52--mcpmodel-context-protocol-基础)。
...
2. **跨到 Track B**（学怎么写自己的 agent）
   ...
   - Stage 5 深挖 Antigravity CLI 内部运作
