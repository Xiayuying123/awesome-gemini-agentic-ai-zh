...
> 🚀 **第一次裝 Antigravity CLI / 寫 `GEMINI.md` / `SKILL.md`？** 快速 setup 指南在 [`resources/setup-guide.md` D-E](../resources/setup-guide.md)。已經熟可以跳過。
...
|---|---|---|---|
| **AI 結對程式設計** | 寫到一半忘 syntax / 想到 method 名 | 自動補完 + 改寫 + 解釋 | Cursor / Copilot → Antigravity CLI |
| **多檔重構** | 改一個 class 怕漏改、跨檔 rename 易錯 | batch refactor、改 50 個檔保持風格一致 | Cursor → Antigravity CLI → codex-delegate |
| **Code review（自己 PR）** | review 自己的 diff 看不出問題 | 找 bug / smell、檢查 edge case | Antigravity CLI / cline → Continue（CI） |
| **寫 test** | TDD 一直忘加 case、coverage 不足 | 從 signature / spec 生 pytest | Antigravity CLI + Aider |
| **Debug** | log 不夠、stack trace 看不懂 | 解 trace、生 hypothesis、跑 minimal repro | Antigravity CLI |
| **Doc** | docstring / README 沒人寫、refactor 後過期 | 從 code 生 doc、PR 對應改 doc | Antigravity CLI |
| **CI / 團隊自動化** | 重複手動跑 review、跨人風格不一 | GitHub Action 自動跑 review / lint | Antigravity CLI Action + Continue |

> 💡 **個人 vs 團隊**：表中前 6 個是個人 daily workflow；最後 1 個（CI）是團隊規範。團隊規模 < 5 人時 CI 自動化的 ROI 不高、可先不上。

## 精選 Projects

> **CLI agent 比較**：7 個主流 CLI agent（Antigravity CLI / Codex / OpenCode / Gemini CLI / goose / Aider / Hermes Agent）的並列比較見 [`resources/cli-agents-guide.md`](../resources/cli-agents-guide.md)。第一次接觸 CLI agent 想要 step-by-step 入門 → [`tracks/cli/A1-cli-intro.md`](../tracks/cli/A1-cli-intro.md)（Track A 第一站）。
>
...
★ 72k+ · MIT — open source 的自主軟體開發 agent。設計上比 Aider / Antigravity CLI 更激進——agent 自己跑 sandbox、自己 commit，適合「整個 issue 丟給它解」場景。
...
- [**yamadashy/repomix**](https://github.com/yamadashy/repomix) ⭐⭐⭐⭐⭐ ★ 24k+ — **典型開發者用途：打包整個 codebase 給 reviewer / refactor agent**。輸出單一 AI-friendly 檔案（XML / Markdown / JSON），方便 Antigravity CLI / Codex 做 code review / refactoring。技術細節（MCP server mode、tree-sitter 壓縮、secretlint 過濾）見官方 README。**Track A 很值得當 daily driver 的工具。**

## 必練流程（按使用頻率）

| 頻率 | 流程 | 怎麼做（≤ 3 步） | 推薦工具 | 適合誰 |
|---|---|---|---|---|
| **每天** | AI 結對寫 code | (1) 開 branch<br>(2) 任務丟給 Antigravity CLI、**先 plan**（不寫 code）<br>(3) Review plan → approve → 寫 code → 自己 review diff | Antigravity CLI / Cursor / Cline | 全開發者 |
| **每天** | Git-native AI 編輯 | (1) `aider`<br>(2) 自然語言請求<br>(3) review + commit / `/undo` | Aider | 想要乾淨 git 流程的人 |
| **Per PR** | 自動 code review | (1) `.github/workflows/claude-review.yml`<br>(2) 抓 git diff → 跑 prompt → post 回 PR<br>(3) human + AI 雙審 | Antigravity CLI Action + Continue | 團隊 |
| **Per feature** | 測試生成 | (1) 給 function signature + docstring<br>(2) 請 AI 生 pytest case（含 edge case）<br>(3) 跑覆蓋率 + 故意改 bug 看 test 抓不抓得到 | Antigravity CLI / Aider | 寫 test 階段 |
...
2. 把任務丟給 Antigravity CLI / Cursor，**先讓它寫 plan**（不直接寫 code）
...
| 把 .env / API key 丟給 AI 看 | 用工具對應的排除機制：Cursor `.cursorignore` / Aider `.aiderignore` / Antigravity CLI 用 `.claude/settings.json` 的 `permissions.deny` |
...
- **要寫自己的 Skill / MCP server** → [Stage 5](../stages/05-gemini-skills-ecosystem.md) + [`resources/cookbook.md`](../resources/cookbook.md)
...
- IDE-specific 設定範本（Cursor `.cursorrules`、Antigravity CLI `GEMINI.md` for Python / Go / Rust 等）
