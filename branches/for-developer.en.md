...
> 🚀 **First time installing Antigravity CLI or writing `GEMINI.md` / `SKILL.md`?** The quick setup guide is [`resources/setup-guide.en.md` D-E](../resources/setup-guide.en.md). Skip it if you already know this.
...
|---|---|---|---|
| **AI pair programming** | You forget syntax mid-flow or cannot recall a method name | Autocomplete + rewrite + explanation | Cursor / Copilot → Antigravity CLI |
| **Multi-file refactoring** | Changing one class risks missed references; cross-file rename is error-prone | Batch refactors while keeping style consistent across many files | Cursor → Antigravity CLI → codex-delegate |
| **Code review (your own PR)** | Reviewing your own diff makes it easy to miss problems | Find bugs / smells and check edge cases | Antigravity CLI / Cline → Continue (CI) |
| **Writing tests** | TDD cases are easy to miss; coverage falls short | Generate pytest cases from signatures / specs | Antigravity CLI + Aider |
| **Debugging** | Logs are thin; stack traces are hard to interpret | Explain traces, generate hypotheses, run minimal repros | Antigravity CLI |
| **Docs** | Docstrings / READMEs lag behind refactors | Generate docs from code and update docs alongside PRs | Antigravity CLI |
| **CI / team automation** | Manual review is repetitive; style varies across people | Run automated review / lint in GitHub Actions | Antigravity CLI Action + Continue |
...
> **CLI agent comparison**: 7 major CLI agents (Antigravity CLI / Codex / OpenCode / Gemini CLI / goose / Aider / Hermes Agent) compared side-by-side in [`resources/cli-agents-guide.en.md`](../resources/cli-agents-guide.en.md). New to CLI agents and want step-by-step onboarding → [`tracks/cli/A1-cli-intro.en.md`](../tracks/cli/A1-cli-intro.en.md) (Track A first stop).
>
...
★ 72k+ · MIT — open-source autonomous software development agent. More aggressive design than Aider / Antigravity CLI — agent runs in its own sandbox and commits autonomously. Best for "throw a whole issue at it" scenarios.
...
- [**yamadashy/repomix**](https://github.com/yamadashy/repomix) ⭐⭐⭐⭐⭐ ★ 24k+ — **Typical developer use case: package the whole codebase for a reviewer / refactor agent**. Outputs a single AI-friendly file (XML / Markdown / JSON) for Antigravity CLI / Codex code review / refactoring. See the official README for technical details such as MCP server mode, tree-sitter compression, and secretlint filtering. **A must-have, daily-driver-grade tool for Track A.**

## Workflows to Master (by frequency)

| Frequency | Workflow | Steps (≤3) | Recommended tools | Best for |
|---|---|---|---|---|
| **Daily** | AI pair programming | (1) Open a branch<br>(2) Give the task to Antigravity CLI and **ask for a plan first** (no code yet)<br>(3) Review plan → approve → code → review your own diff | Antigravity CLI / Cursor / Cline | All developers |
...
| **Per PR** | Automated code review | (1) `.github/workflows/claude-review.yml`<br>(2) Capture git diff → run prompt → post back to PR<br>(3) human + AI review | Antigravity CLI Action + Continue | Teams |
| **Per feature** | Test generation | (1) Provide function signature + docstring<br>(2) Ask AI for pytest cases, including edge cases<br>(3) Run coverage + intentionally break a bug to verify tests catch it | Antigravity CLI / Aider | Test-writing phase |
...
2. Hand the task to Antigravity CLI / Cursor — **make it write a plan first** (don't dive into code)
...
| Hand `.env` / API keys to the AI | Use your tool's exclusion mechanism — Cursor `.cursorignore` / Aider `.aiderignore` / Antigravity CLI `permissions.deny` in `.claude/settings.json` |
...
| **Tier 1** | Antigravity CLI / Cline / OpenCode + `GEMINI.md` | CLI with file-system access, human-in-the-loop | 1-2 days |
...
- **Author your own Skill / MCP server** → [Stage 5](../stages/05-gemini-skills-ecosystem.en.md) + [`resources/cookbook.en.md`](../resources/cookbook.en.md)
...
- IDE-specific config templates (Cursor `.cursorrules`, Antigravity CLI `GEMINI.md` for Python / Go / Rust, etc.)
