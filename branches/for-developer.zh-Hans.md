...
> 🚀 **第一次装 Antigravity CLI / 写 `GEMINI.md` / `SKILL.md`？** 快速 setup 指南在 [`resources/setup-guide.zh-Hans.md` D-E](../resources/setup-guide.zh-Hans.md)。已经熟可以跳过。
...
|---|---|---|---|
| **AI 结对编程** | 写到一半忘 syntax / 想不到 method 名 | 自动补全 + 改写 + 解释 | Cursor / Copilot → Antigravity CLI |
| **多文件重构** | 改一个 class 怕漏改、跨文件 rename 容易错 | batch refactor、改 50 个文件仍保持风格一致 | Cursor → Antigravity CLI → codex-delegate |
| **Code review（自己 PR）** | review 自己的 diff 看不出问题 | 找 bug / smell、检查 edge case | Antigravity CLI / Cline → Continue（CI） |
| **写 test** | TDD 常忘加 case、coverage 不足 | 从 signature / spec 生成 pytest | Antigravity CLI + Aider |
| **Debug** | log 不够、stack trace 看不懂 | 解释 trace、生成 hypothesis、跑 minimal repro | Antigravity CLI |
| **Doc** | docstring / README 没人写、refactor 后过期 | 从 code 生成 doc、PR 对应改 doc | Antigravity CLI |
| **CI / 团队自动化** | 重复手动跑 review、跨人风格不一 | GitHub Action 自动跑 review / lint | Antigravity CLI Action + Continue |

> 💡 **个人 vs 团队**：表中前 6 个是个人 daily workflow；最后 1 个（CI）是团队规范。团队规模 < 5 人时 CI 自动化的 ROI 不高，可以先不上。

## 精选 Projects

> **CLI agent 比较**：7 个主流 CLI agent（Antigravity CLI / Codex / OpenCode / Gemini CLI / goose / Aider / Hermes Agent）的并列比较见 [`resources/cli-agents-guide.zh-Hans.md`](../resources/cli-agents-guide.zh-Hans.md)。第一次接触 CLI agent 想要 step-by-step 入门 → [`tracks/cli/A1-cli-intro.zh-Hans.md`](../tracks/cli/A1-cli-intro.zh-Hans.md)（Track A 第一站）。
>
...
★ 72k+ · MIT — open source 的自主软件开发 agent。设计上比 Aider / Antigravity CLI 更激进——agent 自己跑 sandbox、自己 commit，适合“整个 issue 丢给它解”场景。
...
- [**yamadashy/repomix**](https://github.com/yamadashy/repomix) ⭐⭐⭐⭐⭐ ★ 24k+ — **典型开发者用途：打包整个 codebase 给 reviewer / refactor agent**。输出单个 AI-friendly 文件（XML / Markdown / JSON），方便 Antigravity CLI / Codex 做 code review / refactoring。技术细节（MCP server mode、tree-sitter 压缩、secretlint 过滤）见官方 README。**Track A 的必备 daily-driver 工具。**

## 必练流程（按使用频率）

| 频率 | 流程 | 怎么做（≤ 3 步） | 推荐工具 | 适合谁 |
|---|---|---|---|---|
| **每天** | AI 结对写 code | (1) 开 branch<br>(2) 任务丢给 Antigravity CLI、**先 plan**（不写 code）<br>(3) Review plan → approve → 写 code → 自己 review diff | Antigravity CLI / Cursor / Cline | 全开发者 |
| **每天** | Git-native AI 编辑 | (1) `aider`<br>(2) 自然语言请求<br>(3) review + commit / `/undo` | Aider | 想要干净 git 流程的人 |
| **Per PR** | 自动 code review | (1) `.github/workflows/claude-review.yml`<br>(2) 抓 git diff → 跑 prompt → post 回 PR<br>(3) human + AI 双审 | Antigravity CLI Action + Continue | 团队 |
| **Per feature** | 测试生成 | (1) 给 function signature + docstring<br>(2) 请 AI 生成 pytest case（含 edge case）<br>(3) 跑覆盖率 + 故意改 bug 看 test 抓不抓得到 | Antigravity CLI / Aider | 写 test 阶段 |
...
2. 把任务丢给 Antigravity CLI / Cursor，**先让它写 plan**（不直接写 code）
...
| 把 .env / API key 丢给 AI 看 | 用工具对应的排除机制：Cursor `.cursorignore` / Aider `.aiderignore` / Antigravity CLI 用 `.claude/settings.json` 的 `permissions.deny` |
...
| **Tier 1** | Antigravity CLI / Cline / OpenCode + `GEMINI.md` | CLI 接 file system、human-in-the-loop | 1-2 天上手 |
...
- **要写自己的 Skill / MCP server** → [Stage 5](../stages/05-gemini-skills-ecosystem.zh-Hans.md) + [`resources/cookbook.zh-Hans.md`](../resources/cookbook.zh-Hans.md)
...
- IDE-specific 设置范本（Cursor `.cursorrules`、Antigravity CLI `GEMINI.md` for Python / Go / Rust 等）
