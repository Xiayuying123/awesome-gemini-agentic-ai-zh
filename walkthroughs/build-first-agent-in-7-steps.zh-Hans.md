...
| 5 | 包成 Antigravity CLI Skill | SKILL.md + 30 行 |
...
## Stage 5 — 包成 Antigravity CLI Project Skill

> 这一步**不是** Python，是把前面 Stage 1-4 的逻辑，重新包成 Antigravity CLI 自己会加载的 **project skill**。`description` 写得清楚的话，Claude 会在用户提到相关需求时自动触发。
...
## How to do it
1. 从 URL 抓 paper 内容（用 Antigravity CLI 内建的 WebFetch tool；或在用户贴了 PDF 时用 Read tool）
2. 套用以下 prompt 结构：
   - 动机（≤60 字）
   - 方法（≤60 字）
   - 结果（≤60 字）
...
放好后，**在这个 repo 里开 Antigravity CLI**——project-level skill 会自动加载（不需要安装指令）。Claude 看到 description 跟用户输入吻合就会用这个 skill。

验证它是否生效：在 Antigravity CLI 对话里贴 `https://arxiv.org/abs/2210.03629`，看 Claude 是不是按你定义的格式响应。
...
**进阶**：如果想把这个 skill 包成可分享的 plugin（让别人也能装在自己的 Antigravity CLI），参考 [Stage 5.4 Plugins & Marketplaces](../stages/05-gemini-skills-ecosystem.zh-Hans.md#54--plugins-与-marketplaces)。本 walkthrough 不展开 plugin 打包流程。
...
- [ ] 把 agent 包成 Antigravity CLI skill（Stage 5）
